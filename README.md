...
To reuse a prompt cache generated by `vlm-rknn-cache`, pass its path with `--cache`. The CLI loads the cache at startup and does not update it during inference:
...
./vlm-rknn \
  ...
  --prompt "Query that follows the cached prefix"
```
